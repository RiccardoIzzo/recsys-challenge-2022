...
def find_fav_type(user, urm, icm):
    user_item = urm.query('user_id == @user and data == 0').groupby(['item_id']).count().sort_values('user_id',
                                                                                                     ascending=False)
...
def create_new_df(urm, icm_l, icm_t):
    ...
    return pd.DataFrame(df,
                        columns=['user_id', 'item_id', 'n_0', 'n_1', 'avg_watched_ep', 'ep_tot', 'type', 'impressions',
                                 'fav_type'])
