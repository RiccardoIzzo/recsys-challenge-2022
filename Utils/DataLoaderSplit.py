...
class DataLoaderSplit:
    def __init__(self, dataset_dir: str = "data", test_split=0.2, val_split=0.1, urm='URM_df.csv'):
        ...
        master_df = pd.read_csv('../data/other_stuff/Master_df.csv', engine='python')

        ICM_length_path = '../data/other_stuff/Complete_ICM_length_OLD.csv'
        ICM_type_path = '../data/other_stuff/Complete_ICM_type.csv'
        ...
        self.users_to_recommend = pd.read_csv('../' + dataset_dir + '/data_target_users_test.csv')[
    ...
    def get_user_feature(self):
        ...
        avg_coo = sps.coo_matrix((users['avg_watched_ep'], (users['user_id'], users['avg_watched_ep'])),
                                 shape=(n_users, int(np.max(users['avg_watched_ep'])) + 1))
        fav_coo = sps.coo_matrix((users['fav_type'], (users['user_id'], users['fav_type'])),
                                 shape=(n_users, int(np.max(users['fav_type'])) + 1))
        n_0_coo = sps.coo_matrix((users['n_0'], (users['user_id'], users['n_0'])),
                                 shape=(n_users, int(np.max(users['n_0'])) + 1))
        n_1_coo = sps.coo_matrix((users['n_1'], (users['user_id'], users['n_1'])),
                                 shape=(n_users, int(np.max(users['n_1'])) + 1))
